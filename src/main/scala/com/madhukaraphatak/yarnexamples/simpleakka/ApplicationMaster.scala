...
object ApplicationMaster {
   ...
   def main(args: Array[String]) {
     ...
     while( completedContainers < n) {
       ...
       setUpAppMasterJar(new Path(taskFilePath),taskFileResource)*/
       ...
       for (container <- response.getAllocatedContainers.asScala) {
         ...
         ctx.setCommands(
           List(
             "$JAVA_HOME/bin/java" +
               " -Xmx256M" +
               " com.madhukaraphatak.yarnexamples.akka.TaskActor" +
         ...
         //localResources += "actorRef" -> taskFileResource
